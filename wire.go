...
func ListenAndServe(address string, handler SimpleQueryFn, parseHanlder ParseFn) error {
	server, err := NewServer(SimpleQuery(handler), Parse(parseHanlder))
	if err != nil {
		return err
	}
...
type Server struct {
	...
	preparedStatements map[string]PreparedStatement
	preparedPortals    map[string]pgproto3.Bind
	closer             chan struct{}
}
